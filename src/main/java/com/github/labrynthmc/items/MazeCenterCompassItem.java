...
public class MazeCenterCompassItem extends Item {
	public MazeCenterCompassItem(Properties properties) {
		super(properties);
			...
			public float call(ItemStack p_call_1_, @Nullable World p_call_2_, @Nullable LivingEntity p_call_3_) {
				if (p_call_3_ == null && !p_call_1_.isOnItemFrame()) {
					...
					if (p_call_2_.dimension.isNether()) {
						...
						double d2 = this.getAngleToCenter(p_call_2_, entity) / (double)((float)Math.PI * 2F);
						d0 = 0.5D - (d1 - 0.25D - d2);
					} else {
						d0 = Math.random();
					}
